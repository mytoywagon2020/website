...
- **`shopify/educator-portal/EDUCATOR-PORTAL-RUNBOOK.md`** — the operating design. Re-read it before educator-portal work. Key invariants:
  ...
  - **Default onboarding is the EASY path** (no app/token/Flow/dev): Helium saves applicant info to the customer → verify → add `educator-approved` tag (one click) → gate opens + Educators Market prices automatically; make a B2B Company only when PO/Net-30 is needed. Flow auto-populate is an optional later upgrade.
