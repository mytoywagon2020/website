...
4. **Procurement self-justification.** AP/grant teams need one-click **PDF quote/invoice, W-9, COI, PO reference**. TODO: add real file URLs for W-9/COI to the vendor-profile + dashboard (currently links to the page + email request). Shopify order pages provide printable invoices; draft-order invoices cover quotes.
