...
3. **Degraded image refs (owner-flagged).** Sensory uses `file_url` (20 refs); some may point to filenames no longer in Files. SUSPECTED broken (verify against full Files list): `Rainbow_Play_Tray.webp`, `wooden-story-montessori-sand-tray.png`, `wooden-story-2-part-sand-tray-flashcard-holder.png`, `wooden-story-sand-tray-tools-set-of-4.png`, `buttonandbug-cake-sensory-tray.png` (exists as a theme ASSET, not in Files — `file_url` would break). **TODO: pull the full Files inventory and diff every `file_url`/`asset_url` ref; repoint or re-upload.** Other sections use `asset_url` (theme assets) which are present.
